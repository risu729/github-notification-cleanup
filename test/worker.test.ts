...
describe("notification discovery", () => {
  ...
  test("uses the previous checkpoint for incremental discovery", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
      ...
      expect(url.searchParams.get("since")).toBe(previousCheckpoint);
    ...
    expect(sendBatch).toHaveBeenCalledWith([{ body: notification("1", 1), contentType: "json" }]);
    const [run] = await loadRuns();
    expect(run?.since).toBe(previousCheckpoint);
  });
...
describe("notification Queue consumer", () => {
  ...
  test.each([
    ...
    expect(audit).toEqual({ outcome: "marked_done", reason: "renovate_merged" });
    const [run] = await loadRuns();
  ...
  test("ignores an automation command mention around its bot response", async () => {
    const botNotification: Notification = {
      ...notification("1", 11_720),
      subjectUrl: "https://api.github.com/repos/jdx/mise/pulls/11720",
    };
    const botThread = {
      ...thread("1", 11_720),
      subject: {
        type: "PullRequest",
        url: botNotification.subjectUrl,
      },
    };
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
      const url =
      ...
      if (pathname === "/notifications/threads/1" && method === "GET") {
        return response(botThread);
      }
      if (pathname === "/repos/jdx/mise/pulls/11720") {
        return response(
          pullRequest(11_720, {
            html_url: "https://github.com/jdx/mise/pull/11720",
            state: "open",
            user: { id: 79_110_363 },
          }),
        );
      }
      if (pathname === "/repos/jdx/mise/issues/11720/timeline") {
        return response([
          {
            actor: { id: 79_110_363 },
          ...
          {
            actor: { id: 132_028_505 },
            created_at: "2026-08-04T00:01:00Z",
            event: "mentioned",
          },
          {
            actor: { id: 136_622_811 },
            body: "Reviews resumed.",
            created_at: "2026-08-04T00:01:10Z",
            event: "commented",
          },
        ]);
      }
    ...
    vi.spyOn(console, "log").mockImplementation(() => undefined);
    const batch = queueBatch([botNotification]);
    const context = createExecutionContext();
    ...
    expect(audit).toEqual({ outcome: "marked_done", reason: "ignored_bot_review" });
    const [run] = await loadRuns();
    expect(JSON.parse(run?.summary ?? "null")).toMatchObject({ botReviewMarkedDone: 1 });
  });
