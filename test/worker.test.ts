...
import { forceCheckKey, notificationStateKey } from "../src/state";
...
describe("scheduled Worker state", () => {
  beforeEach(async () => {
    await Promise.all([env.STATE.delete(forceCheckKey), env.STATE.delete(notificationStateKey)]);
  });
  ...
  test("stores the successful check time as JSON", async () => {
    ...
    const state = await env.STATE.get<{ lastCheckedAt: string }>(notificationStateKey, "json");
  ...
  test("does not advance the state after a GitHub API failure", async () => {
    ...
    await env.STATE.put(notificationStateKey, JSON.stringify(previousState));
    ...
    await expect(
      env.STATE.get<{ lastCheckedAt: string }>(notificationStateKey, "json"),
    ).resolves.toEqual(previousState);
  });

  test("retains a queued full-check request after failure", async () => {
    await env.STATE.put(forceCheckKey, "true");
    vi.spyOn(globalThis, "fetch").mockResolvedValue(response({ message: "failure" }, 500));

    await expect(runNotificationCleanup(env)).rejects.toThrow();

    await expect(env.STATE.get(forceCheckKey)).resolves.toBe("true");
  });
  ...
  test("consumes a queued full-check request after success", async () => {
    ...
    await expect(env.STATE.get(forceCheckKey)).resolves.toBeNull();
  });

  test("replaces malformed JSON state after a full scan", async () => {
    mockGitHub();
    await env.STATE.put(notificationStateKey, "{");
    ...
    expect(
      warning.mock.calls.some(([message]) =>
        String(message).includes("notification_state_malformed"),
      ),
    ).toBe(true);
    const state = await env.STATE.get<{ lastCheckedAt: string }>(notificationStateKey, "json");
    expect(Date.parse(state?.lastCheckedAt ?? "invalid")).not.toBeNaN();
  ...
  test("replaces structurally invalid state after a full scan", async () => {
    mockGitHub();
    await env.STATE.put(notificationStateKey, JSON.stringify({ lastCheckedAt: 1 }));
    const warning = vi.spyOn(console, "warn").mockImplementation(() => undefined);
